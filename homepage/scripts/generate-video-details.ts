...
async function main() {
  ...
  /** Shorts + HIDDEN — never generate detail pages (duration is authoritative; tags are not). */
  ...
  if (args.id) {
    const pool = metadata.videos.filter((v) => v.id === args.id)
    selected = pool.filter((v) => !isExcluded(v))
