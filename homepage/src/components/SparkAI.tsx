...
export function SparkAI() {
  ...
  const [savedSparks, setSavedSparks] = useLocalStorage<SavedSpark[]>(
    STORAGE_KEY,
    [],
  );
  ...
  const generateSpark = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model:
          import.meta.env.VITE_GEMINI_API_MODEL || "gemini-3-flash-preview",
    ...
    >
      ...
      >
        <div className="text-center mb-16">
          <h2 className="text-5xl italic mb-6 font-serif text-pastel-dark">
            Let's follow an idea...
          </h2>
          <p className="text-pastel-dark/50 text-xl font-serif">
            Enter something you're curious about, and let's see how it seeds the
            garden.
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-20 items-center">
          ...
          <SketchButton
            ...
            disabled={loading}
            className="min-w-60 w-full md:w-auto"
          >
            {loading ? (
              <>
                <svg
        ...
        {error && (
          <p className="text-center text-pastel-terracotta mb-12 animate-fade-in font-serif italic text-lg">
            {error}
          </p>
        )}
                  ...
                  {[
                    {
                      icon: "💡",
                      title: "The Spark",
                      desc: `Learns ${result.topic}`,
                      color: "text-pastel-terracotta",
                      bg: "bg-pastel-terracotta/5",
                      border: "border-pastel-terracotta/20",
                    },
                    {
                      icon: "🎨",
                      title: "The Creation",
                      desc: result.creation,
                      color: "text-pastel-tan",
                      bg: "bg-pastel-tan/5",
                      border: "border-pastel-tan/20",
                    },
                    {
                      icon: "📺",
                      title: "The Platform",
                      desc: result.platform,
                    ...
                    },
                    {
                      ...
                      title: "The Impact",
                      desc: result.impact,
                      color: "text-pastel-medium",
                      bg: "bg-pastel-medium/5",
                      border: "border-pastel-medium/20",
                    },
                  ].map((item, i) => (
                    <motion.div
                      key={i}
                    ...
                    >
                      <div
                        className={`w-20 h-20 mx-auto ${item.bg} rounded-full flex items-center justify-center mb-6 text-3xl sketch-border ${item.border} transition-all group-hover:scale-110 group-hover:shadow-md duration-300 transform ${i % 2 === 0 ? "rotate-3" : "-rotate-3"}`}
                      >
                        {item.icon}
                      </div>
                      <h4
                        className={`font-bold text-xs uppercase ${item.color} mb-3 tracking-[0.2em]`}
                      >
                        {item.title}
                      </h4>
                      <p className="text-sm text-pastel-dark/70 leading-relaxed font-serif italic">
                        {item.desc}
                      </p>
                    </motion.div>
                  ))}
        ...
        {savedSparks.length > 0 && (
          <div className="mt-20 border-t border-pastel-dark/5 pt-16">
            <div className="flex justify-between items-center mb-10">
              <div>
                <h3 className="text-3xl italic text-pastel-dark mb-2">
                  The Seed Collection
                </h3>
                <p className="text-sm text-pastel-dark/40 font-serif lowercase italic">
                  Ideas planted for future growth.
                </p>
              </div>
              <div className="text-right">
                <span className="text-[10px] font-bold text-pastel-medium/40 uppercase tracking-[0.3em] block mb-1">
                  Status
                </span>
                <span className="text-xs font-bold text-pastel-medium uppercase tracking-widest">
                  {savedSparks.length} SEEDS PLANTED
                </span>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="mb-6">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest text-pastel-terracotta/60">
                        Subject of Interest
                      </span>
                    </div>
                    <p className="text-lg font-serif italic text-pastel-dark leading-tight">
                      {spark.topic}
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-pastel-dark/5">
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-pastel-tan/60 block mb-1">
                        Proposed Creation
                      </span>
                      <p className="text-xs text-pastel-dark/70 leading-relaxed italic">
                        {spark.creation}
                      </p>
                    </div>
                    <div className="flex justify-between items-end">
                      <div>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-pastel-medium/60 block mb-1">
                          Platform
                        </span>
                        <p className="text-[10px] text-pastel-medium font-bold uppercase tracking-tighter">
                          {spark.platform}
                        </p>
                      </div>
                      <span className="text-[10px] text-pastel-dark/20 font-serif italic">
                        {new Date(spark.date).toLocaleDateString(undefined, {
                          month: "short",
                          day: "numeric",
                        })}
                      </span>
